...
export default function removeMissingResults() {
    ...
    this.raw_data.forEach(d => {
        if (/^\s*$/.test(d[this.config.value_col]))
            missingResults.push(d);
        else
            nonMissingResults.push(d);
    });
    ...
    this.removedRecords.missing = sum(
        participantsWithMissingResults.filter(d => this.removedRecords.placeholderRecords.indexOf(d.key) < 0),
        d => d.values
    );
    if (this.removedRecords.missing > 0)
        console.warn(
            `${this.removedRecords.missing} record${
                this.removedRecords.missing > 1 ? 's with a missing result have' : ' with a missing result has'
            } been removed.`
        );
