...
const dataArray = [
    {
        ...
        path:
            'https://raw.githubusercontent.com/RhoInc/data-library/master/data/clinical-trials/sdtm/cdisc-pilot-01/dm.csv'
    },
    {
        type: 'AE', // adverse events
        path:
            'https://raw.githubusercontent.com/RhoInc/data-library/master/data/clinical-trials/sdtm/cdisc-pilot-01/ae.csv'
    },
    {
        type: 'BDS', // basic data structure: labs
        path:
            'https://raw.githubusercontent.com/RhoInc/data-library/master/data/clinical-trials/sdtm/cdisc-pilot-01/lb.csv'
    },
    {
        type: 'BDS', // basic data structure: vital signs
        path:
            'https://raw.githubusercontent.com/RhoInc/data-library/master/data/clinical-trials/sdtm/cdisc-pilot-01/vs.csv'
    }
];

//Get safety-explorer-suite version from window hash.
const version = window.location.hash ? window.location.hash.substring(1) : 'master';
if (version !== 'master') loadLibrary(version);
...
branchRequest.open('get', 'https://api.github.com/repos/RhoInc/safety-explorer-suite/branches');
branchRequest.send();
...
releaseRequest.open('get', 'https://api.github.com/repos/RhoInc/safety-explorer-suite/releases');
releaseRequest.send();

//Add version select functionality.
d3.select('.version-select__submit').on('click', function() {
    const version = d3
        .select('.version-select__select')
...
function loadLibrary(version) {
    ...
    const versionURL = version !== 'master' ? baseURL + '@' + version : baseURL;
    ...
    pkgRequest.open('get', versionURL + '/package.json');
    pkgRequest.send();
}
