(function (global, factory) {
    ...
    function syncSettings(settings) {
        ...
        array_settings.forEach(function (s) {
            if (!(settings[s] instanceof Array)) settings[s] = typeof settings[s] === 'string' ? [settings[s]] : [];
        });
    ...
    function identifyControls() {
        ...
        controlGroups.attr('id', function (d) {
            ...
            controlGroup.classed(d.type, true).classed(d.type.toLowerCase().replace(' ', '-') + ' ' + d.label.toLowerCase().replace(' ', '-'), true);
        });
    ...
    function customizeGroupByControl() {
        ...
        if (groupControl.datum().values.length === 1) groupControl.style('display', 'none');else groupControl.selectAll('select').on('change', function (d) {
            var label = d3.select(this).selectAll('option:checked').text();
    ...
    function calculateYPrecision() {
        ...
        var step = this.measure.range > 0 ? Math.abs(this.measure.range / 15) // non-zero range
        : this.measure.results[0] !== 0 ? Math.abs(this.measure.results[0] / 15) // zero range, non-zero result(s)
    ...
    function checkPointOverlap(d, chart) {
        ...
        var click_x = d3.select(this).select('circle').attr('cx');
        var click_y = d3.select(this).select('circle').attr('cy');
        var click_r = d3.select(this).select('circle').attr('r');
        var click_id = d.values.raw[0][chart.config.id_col];

        // See if any other points overlap
        var overlap_ids = chart.points.filter(function (f) {
            var point_id = f.values.raw[0][chart.config.id_col];
            var point_x = d3.select(this).select('circle').attr('cx');
            var point_y = d3.select(this).select('circle').attr('cy');
    ...
    function addOverlapNote(d, chart) {
        function showID(d) {
            ...
            var participantDropdown = chart.multiples.controls.wrap.style('margin', 0).selectAll('.control-group').filter(function (d) {
                return d.option === 'selected_id';
            }).select('select').property('value', d);
        ...
        if (chart.overlap_ids.length) {
            var click_id = d.values.raw[0][chart.config.id_col];
            var overlap_div = chart.wrap.insert('div', 'div.multiples').attr('class', 'overlapNote').style('background-color', '#eee').style('border', '1px solid #999').style('padding', '0.5em').style('border-radius', '0.2em').style('margin', '0 0.1em');

            overlap_div.append('span').html('<strong>Note</strong>: ' + chart.overlap_ids.length + (' point' + (chart.overlap_ids.length === 1 ? '' : 's') + ' overlap' + (chart.overlap_ids.length === 1 ? 's' : '') + ' the clicked point for <span class="idLink">') + click_id + '</span>. Click an ID for details: ');
            overlap_div.select('span.idLink').datum(click_id).style('color', 'blue').style('text-decoration', 'underline').style('cursor', 'pointer').on('click', showID).on('mouseover', function (d) {
            ...
            var overlap_ul = overlap_div.append('ul').style('list-style', 'none').style('padding', '0').style('display', 'inline-block');

            overlap_ul.selectAll('li').data(chart.overlap_ids).enter().append('li').style('display', 'inline-block').style('padding-right', '.5em').attr('class', 'idLink').style('color', 'blue').style('text-decoration', 'underline').style('cursor', 'pointer').text(function (d) {
                return d;
            }).on('click', showID).on('mouseover', function (d) {
                clearHovered.call(chart);
                chart.hovered_id = d;
                if (chart.hovered_id !== chart.selected_id) highlightHovered.call(chart);
            }).on('mouseout', function (d) {
