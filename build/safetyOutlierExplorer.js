...
function cleanData() {
    ...
    this.measures = d3$1.set(this.raw_data.map(function (d) {
...
function setYdomain() {
    ...
    //Define y-domain.
    if (this.currentMeasure !== this.previousMeasure) this.config.y.domain = d3$1.extent(this.measure_data.map(function (d) {
...
function highlight() {
    ...
    this.lines.filter(function (d) {
        return [_this.hovered_id, _this.selected_id].indexOf(d.values[0].values.raw[0][_this.config.id_col]) > -1;
    }).moveToFront().select('path').attr('stroke-width', this.config.marks.find(function (mark) {
        return mark.type === 'line';
    ...
    }).moveToFront().select('circle').attr('r', this.config.marks.find(function (mark) {
        return mark.type === 'circle';
...
function addLineEventListeners() {
    ...
    this.lineSuperGroup.delegate('mouseover', 'g', function (d) {
        ...
        context.points.classed('selected', function (d) {
...
function addPointEventListeners() {
    ...
    this.svg.delegate('mouseover', 'g.point', function (d) {
        _this.hovered_id = d.values.raw[0][context.config.id_col];
        highlight.call(_this);
    }).delegate('mouseout', 'g.point', function (d) {
        delete _this.hovered_id;
        clearHighlight.call(_this);
        ...
        //Select line and all points corresponding to selected ID.
        context.lines.classed('selected', function (d) {
