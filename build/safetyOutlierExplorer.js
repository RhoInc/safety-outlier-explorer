...
var controlInputs = [{ label: 'Measure', type: 'subsetter', start: null }, { type: 'dropdown', label: 'X-axis', option: 'x.column', require: true }, { type: 'number', label: 'Lower Limit', option: 'y.domain[0]', require: true }, { type: 'number', label: 'Upper Limit', option: 'y.domain[1]', require: true }];
...
function getValType(data, variable) {
    var values = d3$1.set(data.map(function (m) {
        return m[variable];
    })).values(),
...
function onInit() {
    ...
    var context = this,
        config = this.config,
        allMeasures = d3$1.set(this.raw_data.map(function (m) {
    ...
    if (catMeasures.length) {
        console.warn(catMeasures.length + ' non-numeric endpoint' + (catMeasures.length > 1 ? 's have' : ' has') + ' been removed:' + catMeasures.join(', '));
    }
    ...
    })[0].start = config.start_value || numMeasures[0];
    ...
    this.controls.config.inputs = this.controls.config.inputs.filter(function (d) {
        if (d.type != 'subsetter') {
            ...
            var levels = d3$1.set(chart.raw_data.map(function (f) {
...
function onLayout() {
    ...
    var resetContainer = this.controls.wrap.insert('div', '.control-group:nth-child(3)').classed('control-group y-axis', true).datum({
        type: 'button',
        option: 'y.domain',
        label: 'Y-axis:'
    }),
        resetLabel = resetContainer.append('span').attr('class', 'control-label').style('text-align', 'right').text('Y-axis:'),
...
function onPreprocess() {
    ...
    this.config.x.domain = config.x.type === 'ordinal' ? d3$1.set(measure_data.map(function (d) {
        return d[config.x.column];
    })).values() : d3$1.extent(measure_data, function (d) {
        return +d[config.x.column];
    });
    ...
    if (changedMeasureFlag) {
        ...
        this.controls.wrap.selectAll('.y-axis').property('title', 'Initial Limits: [' + this.config.y.domain[0] + ' - ' + this.config.y.domain[1] + ']');
...
function updateSubjectCount(chart, id_col, selector, id_unit) {
    ...
    var currentObs = d3$1.set(chart.raw_data.filter(function (d) {
        var filtered = false;
...
function addBoxPlot(svg$$1, results, height, width, domain, boxPlotWidth, boxColor, boxInsideColor, fmt, horizontal) {
    ...
    boxplot.selectAll('.boxplot').append('title').text(function (d) {
        return 'N = ' + d.values.length + '\n' + 'Min = ' + d3$1.min(d.values) + '\n' + '5th % = ' + formatx(d3$1.quantile(d.values, 0.05)) + '\n' + 'Q1 = ' + formatx(d3$1.quantile(d.values, 0.25)) + '\n' + 'Median = ' + formatx(d3$1.median(d.values)) + '\n' + 'Q3 = ' + formatx(d3$1.quantile(d.values, 0.75)) + '\n' + '95th % = ' + formatx(d3$1.quantile(d.values, 0.95)) + '\n' + 'Max = ' + d3$1.max(d.values) + '\n' + 'Mean = ' + formatx(d3$1.mean(d.values)) + '\n' + 'StDev = ' + formatx(d3$1.deviation(d.values));
    });
}
...
function onResize() {
    ...
    function clearHighlight() {
        chart.svg.selectAll('.line:not(.selected)').select('path').attr('stroke-width', 0.5);
